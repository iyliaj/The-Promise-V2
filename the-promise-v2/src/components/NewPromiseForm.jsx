...
export function NewPromiseForm({ handleClick, setUserData, playPopSound, isSoundOn, isDesktopOrLaptop }) {
    ...
    return (
        <div className={`new-promise-section ${isDesktopOrLaptop ? "w-full max-w-[900px] h-full" : "w-full h-full mb-5"} bg-secondary rounded-md relative overflow-hidden`}>

            <div className="form-design w-full h-full">
                <svg viewBox="100 45 140 150" fill="none" xmlns="http://www.w3.org/2000/svg" className="object-cover w-full h-full">
            ...
            <div className={`form-elements w-full h-full absolute z-5 top-0 left-0 ${!isDesktopOrLaptop ? "p-4 flex flex-col justify-evenly" : "p-8"}`}>
                <div className={`text-inputs-section w-full ${!isDesktopOrLaptop ? "h-[44%]" : "h-[45%]"} flex flex-col`}>
                    <form id={`new-promise-form`} onSubmit={handleSubmit(onSubmit, onError)}>
                        <section className="">
                            ...
                            })} type="text" id="title" className={`promise-title w-full ${!isDesktopOrLaptop ? "h-6 mt-1" : "h-8 mt-2"} bg-canvas rounded-md p-1 text-dark-primary`} />

                        </section>
                        <section className="mt-2">
                            <label htmlFor="description" className="inline text-canvas">Description</label>
                            ...
                            })} type="text" id="description" className={`promise-decscription w-full ${!isDesktopOrLaptop ? "h-6 mt-1" : "h-8 mt-2"} bg-canvas rounded-md p-1 text-dark-primary`} maxLength="50" />
                        </section>
                    </form>
                </div>
                <div className={`deadline-buttons-section w-full ${!isDesktopOrLaptop ? "h-[46%]" : "h-[50%]"} flex flex-col justify-evenly`}>
                    <section>
                        <label htmlFor="date" className="text-canvas">Date</label>
                        <input {...register("date", {
                    ...
                    <section>
                        <label htmlFor="time" className="text-canvas">Time</label>
                        <input {...register("time", {
                ...
                <div className={`new-promise-btns flex flex-col gap-2 absolute ${isDesktopOrLaptop ? "bottom-13 right-3" : "bottom-6 right-4"}`}>
                    <button
